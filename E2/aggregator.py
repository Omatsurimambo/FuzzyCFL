...
class TrainCluster(object):
    ...
    def plot_accuracy(self, results):
        ...
        plt.legend()
        plt.savefig(os.path.join("output", f'{self.args.dataset}_plot.png'))

        plt.figure(figsize=(8, 6))
        for k_i in range(k):
            test_accuracies = [result['test'][k_i]['acc'] for result in results]
            plt.plot(epochs, test_accuracies, label=f'Cluster {k_i} Test Accuracy', linestyle='-')

        plt.xlabel('Epoch')
        plt.ylabel('Test Accuracy')
        plt.title('Test Accuracy for Cluster Model')
        plt.legend()
        plt.savefig(os.path.join("output", f'{self.args.dataset}_cluster_plot.png'))
    ...
    def train(self, cluster_assign, lr):
        ...
        elif self.args.alg == 'mingledpie':
            cl_ct = [np.sum(np.array(cluster_assign) == k_i ) for k_i in range(k)]
            f=[]
            for i in range(k):
                f.append(round(cl_ct[i] * p))

            for k_i, models in enumerate(local_models):
                if len(models) >0:
                    selected_models= []
                    for other_k_i, other_models in enumerate(local_models):
                        if other_k_i != k_i and len(other_models)>0:
                                selected_models.extend(random.sample(other_models, int(f[other_k_i])))
                    self.aggregate_and_update(models, selected_models, self.models[k_i])
    ...
    def aggregate_and_update(self, local_models, selected_models, global_model):
        ...
        for name, param in global_model.named_parameters():
            weights[name] /= (len(local_models) + len(selected_models))
