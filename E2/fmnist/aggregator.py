...
class TrainCluster(object):
    ...
    def plot_accuracy(self, results):
        ...
        plt.legend()
        plt.savefig(os.path.join("output", f'{self.args.dataset}_plot_{timestamp}.png'))
        ...
        plt.legend()
        plt.savefig(os.path.join("output", f'{self.args.dataset}_cluster_plot__{timestamp}.png'))
    ...
    def lr_schedule(self, epoch):
        ...
        if (epoch == 1 or epoch == 2 or epoch == 3) and self.args.LR_DECAY:
            self.lr = self.lr * 0.52

        if epoch % 10 == 0  and self.args.LR_DECAY:
            self.lr = self.lr * 0.1
