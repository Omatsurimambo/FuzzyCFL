...
class TrainCluster(object):
    ...
    def setup(self):
        ...
        self.result_fname = os.path.join(self.args.project_dir, 'results.pickle')
        self.checkpoint_fname = os.path.join(self.args.project_dir, 'checkpoint.pt')
    ...
    def setup_models(self):
        ...
        for _ in range(k):
            assert self.args.p % 2 == 0 or self.args.p == 1
    ...
    def train(self, cluster_assign, lr):
        ...
        for k_i, models in enumerate(local_models):
            if len(models) > 0:
                self.global_param_update(models, self.models[k_i])
        
        sums = total(k, self.fuzzyIdentity, cluster_assign) 

        cl_ct = [np.sum(np.array(cluster_assign) == k_i ) for k_i in range(k)]
        row_sums = [0] * k
        for m_i in range(m):
            for value in self.fuzzyIdentity[m_i]:
                row_sums[value] += 1

        matrix = construct_matrix(k, p, row_sums, cl_ct, sums)
