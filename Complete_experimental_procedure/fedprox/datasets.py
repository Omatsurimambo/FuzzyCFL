...
class CustomTensor(data.TensorDataset):
    def __init__(self, tensors):
        assert all(tensors[0].size(0) == tensor.size(0) for tensor in tensors), "All tensors must have the same size in the first dimension"
...
class Texas100(data.Dataset):
    def __init__(self, file_path, dataidxs=None, train=True, transform=None, target_transform=None):
        ...
        self.data, self.target = self._build_dataset()
...
class Purchase100(data.Dataset):
    def __init__(self, file_path, dataidxs=None, train=True, transform=None, target_transform=None):
        self.file_path = file_path
        self.dataidxs = dataidxs
        self.train = train
