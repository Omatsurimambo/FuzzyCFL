...
from model import *
from datasets import MNIST_truncated, CIFAR10_truncated, CIFAR100_truncated, ImageFolder_custom, SVHN_custom, FashionMNIST_truncated, CustomTensorDataset, CelebA_custom, FEMNIST, Generated, genData, CustomTensor, Texas100, Purchase100
...
def load_purchase100_data(datadir):
...
def load_texas100_data(datadir):
...
def partition_data(dataset, datadir, logdir, partition, n_parties, beta=0.4):
    ...
    if dataset == 'mnist':
        ...
        X_train, y_train, X_test, y_test = load_texas100_data(datadir)
    elif dataset == 'purchase100':
        X_train, y_train, X_test, y_test = load_purchase100_data(datadir)
    elif dataset == 'svhn':
    ...
    if partition == "homo":
        ...
        if dataset in ('cifar100', 'texas100', 'purchase100'):
            K = 100
        elif dataset == 'tinyimagenet':
            K = 200
...
def get_dataloader(dataset, datadir, train_bs, test_bs, dataidxs=None, noise_level=0, net_id=None, total=0):
    if dataset in ('mnist', 'femnist', 'fmnist', 'cifar10', 'svhn', 'generated', 'covtype', 'a9a', 'rcv1', 'SUSY', 'cifar100', 'tinyimagenet', 'purchase100','texas100'):
        if dataset == 'mnist':
