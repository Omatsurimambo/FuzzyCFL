...
def get_learner(
    ...
    if name == "synthetic":
        ...
        is_binary_classification = False
    elif name == "mnist" or name == "fmnist":
        criterion = nn.CrossEntropyLoss(reduction="none").to(device)
        metric = accuracy
        model = SimpleLinear(h1=200).to(device)
        is_binary_classification = False
    elif name == "texas100":
        criterion = nn.CrossEntropyLoss(reduction="none").to(device)
        metric = accuracy
        model = MLPModelTexas().to(device)
        is_binary_classification = False
    elif name == "purchase100":
        criterion = nn.CrossEntropyLoss(reduction="none").to(device)
        metric = accuracy
        model = MLPModelPur().to(device)
        is_binary_classification = False
...
def get_loaders(type_, root_path, batch_size, is_validation):  #数据类型   路径
    """
...
def get_loader(type_, path, batch_size, train, inputs=None, targets=None):
    ...
    if type_ == "tabular":
        ...
        dataset = SubFMNIST(path, emnist_data=inputs, emnist_targets=targets)
    elif type_ == "texas100":
        dataset = TabularDataset(path)
    elif type_ == "purchase100":
        dataset = TabularDataset(path)
    elif type_ == "femnist":
