...
class SubFMNIST(Dataset):
    """
    Constructs a subset of Fashion MNIST dataset from a pickle file;
    expects pickle file to store list of indices
    ...
    def __init__(self, path, fmnist_data=None, fmnist_targets=None, transform=None):
        """
        :param path: path to .pkl file; expected to store list of indices
        :param emnist_data: EMNIST dataset inputs
        :param emnist_targets: EMNIST dataset labels
        ...
        if fmnist_data is None or fmnist_targets is None:
            self.data, self.targets = get_fmnist()
        else:
            self.data, self.targets = fmnist_data, fmnist_targets
...
def get_fmnist():
    fmnist_path = os.path.join("data", "fmnist", "raw_data")
    assert os.path.isdir(fmnist_path), "Download Fashion MNIST dataset!!"
    ...
    fmnist_data = torch.cat([
        fmnist_train.data,
        fmnist_test.data
    ])

    fmnist_targets = torch.cat([
        fmnist_train.targets,
        fmnist_test.targets
    ])

    return fmnist_data, fmnist_targets
